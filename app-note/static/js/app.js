...
(() => {
  ...
  const state = {
    ...
    filter: { section: 'all', folderId: null, tagId: null, keyword: '', sort: 'newest', dateFilter: null },
    paging: { limit: 10, offset: 0, hasMore: false, loading: false, pendingReset: false },
    view: 'grid',           // 'grid' | 'rows'
    editorDirty: false,
    isSaving: false,
    pendingSave: false,
  ...
  function _getOrCreateLexicalEditor() {
    ...
    if (!window.LexicalNoteEditor) {
      console.error('[NoteStack] LexicalNoteEditor not loaded. Is lexical_editor.js included?');
      return null;
    }
    _lexicalEditor = new window.LexicalNoteEditor('lexical-editor-mount', onEditorChange);
    _lexicalEditor.init();
  ...
  const TAG_COLOR_PALETTE = [
    '#4F6EF7', '#22C55E', '#F59E0B', '#EC4899', '#06B6D4',
    '#A855F7', '#EF4444', '#84CC16', '#F97316', '#14B8A6',
    '#8B5CF6', '#3B82F6', '#10B981', '#EAB308', '#D946EF'
  ];
  ...
  function _getNextUniqueColor() {
    const usedColors = new Set(state.tags.map(t => t.color?.toUpperCase()).filter(Boolean));
  ...
  function openTagCreateColorPicker() {
    ...
    input.type = 'color';
    input.value = (_currentTagColor || '#6B7280').startsWith('#') ? _currentTagColor : '#6B7280';
  ...
  function _refreshTagListManage() {
    ...
    state.tags.forEach(tag => {
      ...
      colorBtn.addEventListener('click', () => openColorPicker(tag.id, tag.color || '#6B7280'));
  ...
  const sectionButtonsDesktop = () => Array.from(document.querySelectorAll('.sidebar__item[data-filter]'));
  const sectionButtonsMobile = () => Array.from(document.querySelectorAll('.mobile-nav__item[data-mobile-filter]'));
  ...
  const _syncLabel = _syncEl ? _syncEl.querySelector('.sync-badge__label') : null;
  ...
  function restoreFilterFromUrl() {
    ...
    state.filter.folderId = Number.isInteger(folderId) && folderId > 0 ? folderId : null;
    ...
    state.filter.sort = ['newest', 'oldest', 'alpha'].includes(sort) ? sort : 'newest';
  ...
  function syncFilterToUrl() {
    ...
    if (state.filter.section !== 'all') params.set('section', state.filter.section);
    if (state.filter.folderId) params.set('folder_id', String(state.filter.folderId));
  ...
  async function loadFolders() {
    ...
    state.collapsedFolderIds = new Set([...state.collapsedFolderIds].filter(id => validFolderIds.has(id)));
  ...
  function renderFolderNav() {
    if (!folderNav || !folderNavBack || !folderNavLabel || !folderNavChips) return;
  ...
  function buildNoteCard(note) {
    ...
    const preview = plainContent.replace(/[#*`>\-_\[\]!|]/g, '').replace(/\n+/g, ' ').trim().slice(0, 160);
    const dateStr = fmtDate((isTrashView ? note.deleted_at : note.updated_at) || note.created_at);
    const folderName = note.folder_name || '';

    // Tag chips with color from state.tags
    const tagHtml = tags.map(t => {
      const tagData = state.tags.find(st => st.name.toLowerCase() === t.toLowerCase());
    ...
    // Footer
    const folderIdAttr = Number.isInteger(note.folder_id) && note.folder_id > 0
    ...
    div.addEventListener('click', e => {
      const tagPill = e.target.closest('.note-card__tag--clickable[data-tag-name]');
      if (tagPill) {
        const clickedTagName = String(tagPill.dataset.tagName || '').trim().toLowerCase();
        const clickedTag = state.tags.find(t => t.name.toLowerCase() === clickedTagName);
      ...
      if (folderMeta) {
        let clickedFolderId = Number.parseInt(String(folderMeta.dataset.folderId || ''), 10);
        if (!Number.isInteger(clickedFolderId) || clickedFolderId <= 0) {
          const clickedFolderName = String(folderMeta.dataset.folderName || '').trim().toLowerCase();
          const clickedFolder = state.folders.find(f => String(f.name || '').trim().toLowerCase() === clickedFolderName);
      ...
      if (e.target.closest('.note-card__star') || e.target.closest('.note-card__menu-btn')) return;
      openNote(note.id);
    });
    ...
    function addFolder(f, depth) {
      ...
      if (hasChildren && chevron) {
        chevron.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          if (state.collapsedFolderIds.has(f.id)) state.collapsedFolderIds.delete(f.id);
  ...
  function syncSectionButtons() {
    sectionButtonsDesktop().forEach(el => {
      el.classList.toggle('sidebar__item--active', el.dataset.filter === state.filter.section);
    });
    sectionButtonsMobile().forEach(el => {
      el.classList.toggle('mobile-nav__item--active', el.dataset.mobileFilter === state.filter.section);
    });
  }
  ...
  function toggleSearchBar() {
    if (!topbarSearchWrap) return;
    const isVisible = topbarSearchWrap.classList.contains('topbar__search-wrap--visible');
  ...
  function setFolderFilter(folderId) {
    state.filter.folderId = state.filter.folderId === folderId ? null : folderId;
    state.filter.tagId = null;
    state.filter.section = 'all';
    syncFilterToUrl();
    syncSectionButtons();
  ...
  function selectNote(id) {
    ...
    document.querySelectorAll('.note-card').forEach(el => {
      el.classList.toggle('note-card--active', parseInt(el.dataset.id, 10) === id);
    });
  }
  ...
  function closeEditor() {
    ...
    document.querySelectorAll('.note-card--active').forEach(el =>
      el.classList.remove('note-card--active'));
  }
  ...
  async function createNoteFromClipboard() {
    ...
    try {
      ...
      alert('Clipboard access unavailable. Make sure the page is served over HTTPS and you have granted clipboard permission.');
      return;
    }
    const body = {
      title:     'New note',
  ...
  async function createNoteSafely() {
    ...
    try {
      ...
      const shouldRetryWithoutFolder = state.filter.folderId != null &&
        (message.includes('folder') || message.includes('constraint') || message.includes('http 500'));
      if (shouldRetryWithoutFolder) {
        try {
          ...
          alert('Could not create a new note. ' + (retryErr?.message || 'Please try again.'));
          return;
        }
      }
      alert('Could not create a new note. ' + (err?.message || 'Please try again.'));
    } finally {
  ...
  function showMainAreaContextMenu(x, y) {
    ...
    showContextMenu(x, y, [
      ...
      { label: '📋  New Note from Clipboard',        action: () => createNoteFromClipboard() },
      'sep',
      { label: '📁  New Folder',                    action: () => openFolderModal(state.filter.folderId || null) },
    ]);
  }

  async function toggleFavorite(note) {
    await api('PUT', `/notes/${note.id}`, { is_favorite: !note.is_favorite ? 1 : 0 });
  ...
  function _folderModalIsDirty() {
    ...
    return current.name !== _folderModalSnapshot.name || current.color !== _folderModalSnapshot.color;
  }
  ...
  function openFolderModal(parentId = null) {
    ...
    if (locationLabel) locationLabel.textContent = _folderLocationLabel(_newFolderParentId);
  ...
  async function saveFolder() {
    ...
    try {
      ...
      alert('Failed to create folder. ' + (err?.message || 'Please try again.'));
    }
  }
  ...
  async function deleteFolder(folderId) {
    const folder = state.folders.find(f => f.id === folderId);
    if (!folder) return;
    if (!confirm(`Delete folder "${folder.name}"? Notes inside will become unfoldered.`)) return;
  ...
  function toggleEditorFullscreen() {
    ...
    const isFullscreen = editorPanel.classList.toggle('editor-panel--fullscreen');
    if (btn) {
  ...
  function showNoteContextMenu(noteId, x, y) {
    ...
    showContextMenu(x, y, [
      ...
      'sep',
      { label: '🏷️  Add Tag…', action: () => { openNote(noteId).then(() => openTagsModal()); } },
      'sep',
      { label: '🗑️  Delete', danger: true, action: () => deleteSingleNote(noteId) },
    ]);
  }

  function showFolderContextMenu(folderId, x, y) {
    showContextMenu(x, y, [
      { label: '📝  Create Note Here',   action: () => createNoteInFolder(folderId) },
      { label: '📁  Create Subfolder',   action: () => openFolderModal(folderId) },
      'sep',
      { label: '✏️  Rename Folder',      action: () => openRenameFolderModal(folderId) },
      { label: '🗑️  Delete Folder',      danger: true, action: () => deleteFolder(folderId) },
    ]);
  }
  ...
  async function saveTags() {
    ...
    if (state.activeNoteId) {
      await api('PUT', `/notes/${state.activeNoteId}`, { tags: tags.join(',') });
  ...
  function bindUI() {
    // New note
    btnNewNote?.addEventListener('click', () => { createNoteSafely(); });
    btnEmptyNew?.addEventListener('click', () => { createNoteSafely(); });

    // Keyboard shortcut Ctrl+N
    document.addEventListener('keydown', e => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'n') { e.preventDefault(); createNoteSafely(); }
      if (e.key === 'Escape') {
        closeSidebar();
        closeEditor();
      }
    ...
    document.querySelectorAll('.mobile-nav__item[data-mobile-filter]').forEach(el => {
      el.addEventListener('click', () => setSectionFilter(el.dataset.mobileFilter));
    });
    ...
    btnMobileNewNote?.addEventListener('click', () => { createNoteSafely(); });
    ...
    $('btn-folder-cancel').addEventListener('click', () => { tryCloseFolderModal(); });
    $('btn-folder-save').addEventListener('click', () => { saveFolder(); });
    $('folder-name-input').addEventListener('keydown', e => { if (e.key === 'Enter') saveFolder(); });
    $('btn-folder-color')?.addEventListener('click', () => $('folder-color-input')?.click());
    $('folder-color-input')?.addEventListener('input', e => {
      _setFolderColor(e?.target?.value || '');
    });

    // Rename folder
    $('btn-rename-folder-cancel').addEventListener('click', () => { $('rename-folder-modal').hidden = true; });
    $('btn-rename-folder-save').addEventListener('click', saveRenameFolder);
    $('rename-folder-input').addEventListener('keydown', e => { if (e.key === 'Enter') saveRenameFolder(); });

    // Editor controls
    $('btn-close-editor').addEventListener('click', closeEditor);
    $('btn-fullscreen-editor')?.addEventListener('click', toggleEditorFullscreen);
    ...
    $('btn-tags-cancel').addEventListener('click', () => { $('tags-modal').hidden = true; });
    ...
    $('tags-input').addEventListener('keydown', e => { if (e.key === 'Enter') saveTags(); });
    ...
    $('btn-tag-manager-close')?.addEventListener('click', () => { $('tag-manager-modal').hidden = true; });
    $('tag-color-btn')?.addEventListener('click', openTagCreateColorPicker);
    $('tag-new-input')?.addEventListener('keydown', e => { if (e.key === 'Enter') createTag(); });

    // Close modals on overlay click
    [$('rename-folder-modal'), $('tags-modal'), $('tag-manager-modal'), $('conflicts-modal')].forEach(overlay => {
    ...
    // Date filter
    $('btn-date-today')?.addEventListener('click', () => setDateFilter(_todayDateStr()));
  ...
  function initSidebarSectionResizer() {
    if (!sidebarBody || !sidebarFolderSection || !sidebarTagSection || !sidebarSectionsResizer) return;
    ...
    sidebarSectionsResizer.addEventListener('keydown', e => {
      ...
      if (e.key === 'ArrowUp') applyFolderHeight(current - 24, { persist: true });
      if (e.key === 'ArrowDown') applyFolderHeight(current + 24, { persist: true });
      if (e.key === 'Home') applyFolderHeight(minPaneHeight, { persist: true });
      if (e.key === 'End') applyFolderHeight(maxFolderHeight(), { persist: true });
    });
