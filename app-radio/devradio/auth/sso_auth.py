...
def init_sso(app):
    ...
    oauth.register(
        "auth_service",
        client_id=app.config.get("AUTHLIB_CLIENT_ID", "radio-app"),
...
def login():
    ...
    session["sso_next"] = _normalize_next_target(request.args.get("next"), url_for("listener.home"))
...
def callback():
    ...
    next_target = _normalize_next_target(session.pop("sso_next", None), url_for("listener.home"))
