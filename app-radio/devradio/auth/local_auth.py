...
def login():
    if current_user.is_authenticated:
        return redirect(url_for("listener.home"))

    if request.method == "POST":
        username = request.form.get("username", "").strip()
        password = request.form.get("password", "")
        user = LocalCredential.query.filter_by(username=username).first()
        if user and user.role == "listener" and user.check_password(password):
            user.ensure_user_profile()
            db.session.commit()
            login_user(user)
            next_target = _normalize_next_target(request.args.get("next"), url_for("listener.home"))
            return redirect(next_target)
        flash("Invalid listener credentials.", "error")

    return render_template("auth/login.html", mode="listener")
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if user and user.role == "admin" and user.check_password(password):
            ...
            db.session.commit()
            login_user(user)
            next_target = _normalize_next_target(request.args.get("next"), url_for("admin.dashboard"))
            return redirect(next_target)
        flash("Invalid admin credentials.", "error")

    return render_template("auth/login.html", mode="admin")
...
def register():
    ...
    if request.method == "POST":
        ...
        user = LocalCredential(username=username, role="listener")
        user.set_password(password)
        db.session.add(user)
        db.session.flush()
        user.ensure_user_profile(timezone_name=timezone_name)
        db.session.commit()
        login_user(user)
        return redirect(url_for("listener.home"))
