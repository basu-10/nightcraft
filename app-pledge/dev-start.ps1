param(
    ...
    [string]$AuthClientId = 'green-pledge-app',
    [string]$AuthClientSecret = 'dev-secret',
    [string]$DatabaseUrl = 'postgresql+psycopg://green_pledge_app:green_pledge_app_db_2026_dev_secret@127.0.0.1:5432/green_pledge_db'
)
...
if (-not $SkipSetup) {
    Write-Host '[dev-start] Running setup (tables/default accounts/pledges)...'
    & $venvPython -m flask --app greenpledge setup
}
else {
    Write-Host '[dev-start] Skipping setup step.'
}

Write-Host '[dev-start] Starting The Green Pledge at http://127.0.0.1:5300'
& $venvPython -m flask --app greenpledge run --port 5300
